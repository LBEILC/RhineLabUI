...
let enabled=false;
const server=createServer(async(req,res)=>{try{let path=decodeURIComponent(new URL(req.url,'http://localhost').pathname);if(path==='/')path='/index.html';let data=await readFile(resolve('release/wallpaper'+path));if(path.endsWith('.html'))data=Buffer.from(data.toString().replace('</head>',`<script>wallpaperPropertyListener.applyUserProperties({load3donstartup:{value:${enabled}},desktopmode:{value:'workbench'},sound:{value:false},music:{value:false}})</script></head>`));res.setHeader('Content-Type',({'.js':'text/javascript','.css':'text/css','.html':'text/html','.json':'application/json','.woff2':'font/woff2'})[extname(path)]||'application/octet-stream');res.end(data)}catch{res.statusCode=404;res.end()}});
...
await page.goto('http://127.0.0.1:5184/');await page.waitForFunction(()=>window.rhine?.stats().startup==='started');
