...
try {for(const browserMotion of ['no-preference','reduce']) {
 ...
 assert.equal(state.motion.reduced,false);
 assert.equal(state.motion.preset,'full');
 await page.evaluate(()=>window.rhine.archive());
 await page.getByRole('button',{name:'系统设置',exact:true}).click();
 assert.equal(await page.locator('.settings-label').textContent(),'设置');
 assert.equal(await page.locator('[data-action="motion-preset"][data-preset="full"]').getAttribute('aria-pressed'),'true');
