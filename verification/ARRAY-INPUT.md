...
- 鼠标左键、触摸与笔使用同一组 Pointer Events。左右拖动切列，上下拖动切当前列的档案；开始移动后锁定主方向。7px 内保留点击，小于 10px 不开始判断拖动方向；已经移动过的手势即使回到起点，也不当作点击。
- 相机投影模式下，沿阵列斜向拖动也可切列或切档。按住时以选中模型中心为采样点，将负 X 的列间距与负 Z 的档案间距投影到真实 CSS 视口；斜向位移沿选中投影向量换算，不套用水平／垂直手势灵敏度。水平和竖直仍作为快捷手势候选；按角度选择最接近的轨道，斜向至少移动 16px，两个候选轴相差不足 3° 时等待到 28px 再确定。每次手势固定投影与轴，反向拖动和松手惯性共用同一映射。上述 1.25 倍规则只用于无相机投影的后备模式。
...
- 水平／竖直快捷手势的灵敏度使用实际 CSS 视口：每列 24% 宽度，限制为 100–280px；每档 14% 高度，限制为 72–150px。与 Three.js 渲染分辨率无关。
...
- `scripts/check-archive-diagonal.mjs`：桌面鼠标、手机竖屏和横屏 CDP 触摸，直接使用实际相机的两条斜向投影，检查正反方向、物理间距跟随和轨道锁定。投影纯逻辑检查补充在 `scripts/check-archive-drag.mjs`，覆盖另一组相机方向、冻结映射，以及横竖手势兼容。报告为 `.tools/array-input/diagonal.json`。
