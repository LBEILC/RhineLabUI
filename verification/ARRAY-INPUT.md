...
- 拖动连续改变原有横向与前后轨道。跨过半格时切换当前档案，沿用标题、声音、波浪、归位和各列记忆；松手后按原有阻尼吸附。用户试用后要求加大惯性：速度投影由 90ms 增至 240ms，上下最多继续三档、左右最多继续两列（原先均为 0.45 格）。松手时将惯性接入原有轨道弹簧，避免先停下再启动；速度限制在不越过吸附目标的范围。停住后松手不追加惯性；减少动态效果时禁用惯性。
