# Win11 桌面动效与 PWA 更新入口（历史记录）

> 以下动效回归记录来自移除系统动效偏好读取之前的版本。当前实现首次进入统一使用完整动画，`prefers-reduced-motion` 不再改变本站的动效模式。

日期：2026-09-09。反馈环境：Windows 11，Chrome 152.0.7977.83，Edge 同样出现症状。

## 复现与判断

在本机 Chrome 152.0.7977.83 和 Edge 152.0.4191.66 中使用新建浏览器上下文，对比 `prefers-reduced-motion` 的两种状态。该历史版本的正常状态进入开机动画；`reduce` 状态直接进入阵列，减少选档、镜头和文字动效。两种状态都可打开右上角圆形图标对应的设置窗口。
...
`scripts/check-startup-motion.mjs` 现在检查不同浏览器动效环境下统一使用完整动画，以及本站的完整 / 减少 / 自定义选择、重播、选档脉冲、正文遮罩和刷新后保留偏好。
