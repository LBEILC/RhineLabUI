...
用户授权三种方案并存。WE「工作台 · 音乐律动 → 音乐律动方案」提供原版多层起伏（默认）、方案 A 鼓点宽波、方案 B 整组轻抬。切换平滑混合，强度滑块和独立呼吸开关继续共用。

新方案根据低频相对慢速平均值的上升触发，最短间隔 220ms，最多保留三道、每道最长 1.8 秒。A 共享同一方向与宽阔空间相位，高频不再独立产生碎波；B 全阵列共用轻抬包络，低中频提供少量持续支撑。使用连续阻尼跟随，沉默时回落。原版函数保持原参数。

检查新增：低频触发、最短间隔、叠加数量、持续音不反复触发、整组同幅、静音归零；均通过，壁纸构建通过。真实歌曲的观感由用户在同一片段切换比较，尚未选定最终版本。验证页同步提供三种切换按钮。
