...
自定义图片显示时，页脚文字与按钮用主题前景色和局部高不透明度底色，关闭 3D 状态不降低文字透明度。无遮罩图片背景实看与时钟、切歌、进度、专注、减少动态效果检查见 scripts/check-workbench-rolling.mjs 和 verification/workbench-rolling。
